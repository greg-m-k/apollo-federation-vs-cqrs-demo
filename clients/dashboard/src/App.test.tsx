...
describe('App', () => {
  ...
  it('renders the header', async () => {
    ...
    expect(screen.getByText('Live Architectural Comparison')).toBeInTheDocument();
    expect(screen.getByText('GraphQL Federation vs Event-Driven Projections')).toBeInTheDocument();
  });
  ...
  it('renders both architecture panels', () => {
    ...
    expect(screen.getByText('Event-Driven Projections')).toBeInTheDocument();
  });
  ...
  it('renders control buttons', () => {
    ...
    expect(screen.getByText('Query Both')).toBeInTheDocument();
  ...
  describe('Federation Query', () => {
    it('queries federation endpoint on button click', async () => {
      global.fetch
        ...
        .mockResolvedValueOnce({ ok: true, json: () => Promise.resolve({}) }) // health check federation
        .mockResolvedValueOnce({ ok: true }) // health check kafka
        .mockResolvedValueOnce({ json: () => Promise.resolve({ data: { person: { name: 'Alice' } } }), headers: { get: () => null } }); // federation query
    ...
    it('handles federation query errors gracefully', async () => {
      global.fetch
        ...
        .mockResolvedValueOnce({ ok: true, json: () => Promise.resolve({}) }) // health check federation
        .mockResolvedValueOnce({ ok: true }) // health check kafka
        .mockRejectedValueOnce(new Error('Network error')); // federation query fails
      ...
      await waitFor(() => {
        expect(global.fetch).toHaveBeenCalledTimes(4); // persons + 2 health checks + federation query
      });
    });
  });
  ...
  describe('Kafka Query', () => {
    ...
    it('handles Kafka query errors gracefully', async () => {
      global.fetch
        ...
        .mockResolvedValueOnce({ ok: true, json: () => Promise.resolve({}) }) // health check federation
        .mockResolvedValueOnce({ ok: true }) // health check kafka
        .mockRejectedValueOnce(new Error('Network error')); // kafka query fails
      ...
      await waitFor(() => {
        expect(global.fetch).toHaveBeenCalledTimes(4); // persons + 2 health checks + kafka query
      });
    });
  });
  ...
  describe('Query Both', () => {
    it('queries both architectures when button clicked', async () => {
      ...
      fireEvent.click(screen.getByText('Query Both'));

      await waitFor(() => {
        expect(global.fetch).toHaveBeenCalledTimes(5); // persons + 2 health checks + federation + kafka
      });
    });
  });
});
